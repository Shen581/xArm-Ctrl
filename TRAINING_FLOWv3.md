...
$$
\mathcal{B}
= \{(o_{b,t}, a_{b,t}) \mid b=1,\ldots,B,\ t=0,\ldots,T-1\}.
$$
...
\begin{aligned}
h_{b,t}
&= [\mathrm{Encoder}(o_{b,t})]_{\mathrm{CLS}}, \\
z_{b,t}
&= \mathrm{Projector}(h_{b,t}).
\end{aligned}
$$
...
$$
u_{b,t}=\mathrm{ActionEncoder}(a_{b,t}),
\qquad
...
$$
\hat{z}_{b,t+1}
= [\mathrm{Predictor}(z_{b,0:t},\ u_{b,0:t})]_{t},
\qquad t=0,\ldots,H-1.
$$
...
$$
\mathrm{err}_{t,m,k}
= (\frac{1}{B}\sum_{b=1}^{B}\cos(\tau_k y_{t,b,m})-\phi(\tau_k))^2
+ (\frac{1}{B}\sum_{b=1}^{B}\sin(\tau_k y_{t,b,m}))^2.
$$
...
B\sum_{k=1}^{K}w_k\mathrm{err}_{t,m,k}.
$$
...
= \mathrm{SIGReg}([z_0,z_1,z_2,z_3]_{\mathrm{batch}}),
\qquad
\mathcal{L}
...
$$
J(a_{0:T-1})
= \|z_{\mathrm{goal}}-\hat{z}_{\mathrm{final}}(a_{0:T-1})\|_2^2.
$$
...
$$
\mathcal{E}
= \mathrm{TopK}_{\mathrm{low}}
(\{J(a^{(s)}_{0:T-1})\}_{s=1}^{S};\ K=30).
$$

然后更新采样分布：

$$
\mu \leftarrow \mathrm{mean}(\mathcal{E}),
\qquad
\sigma \leftarrow \mathrm{std}(\mathcal{E}).
$$

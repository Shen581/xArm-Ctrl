...
class Xarm(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.block_gripper = self.cfg["env"].get("blockGripper", 1)  # True
        ...
        self.reach_threshold = cfg["env"].get("reachThreshold", 0.04)  # 判定阈值 0.03
        # 可视化目标（小球）
        self.target_handles = []
        # 目标位置缓冲区 (x,y,z)
        ...
        # 阻尼系数
        self.damping = 0.05  # 不清楚应该为多少，0.05
        self.ikcount = 0
        # 重置环境
    ...
    def compute_observations(self):
        ...
        if not self.block_gripper:
            fingers_width = self._get_fingers_width()  # 还没改，可能有问题
    ...
    def compute_reward(self, actions):
        ...
        return self.rew_buf  # 这个reach任务用不到
    ...
    def reset_idx(self, env_ids):
        ...
        # 生成新目标位置（新的小球位置）
        rand_range = torch.tensor([0.3, 0.6, 0.3], device=self.device)  # x,y,z范围
    ...
    def _solve_ik_batch(self, target_positions, target_orientations=None):  # =None
        ...
        # 计算旋转误差（默认保持当前朝向）
        #target_orientations = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device)
        #'''
        if target_orientations is None:
            target_orientations = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device)
        target_orientations = target_orientations.unsqueeze(0).repeat(hand_rot.shape[0], 1)
        #'''
        if target_orientations is None:
            orn_err = torch.zeros_like(pos_err)
        else:
            orn_err = self.orientation_error(target_orientations, hand_rot)
        ...
        # 方法1，每次读取gym中的机械臂关节角度
        #'''
        diff = self.dof_pos[:, :6].clone()
        self.gym.refresh_dof_state_tensor(self.sim)
        dof_state_tensor = self.gym.acquire_dof_state_tensor(self.sim)
        dof_state_tensor = gymtorch.wrap_tensor(dof_state_tensor)
        #angles = dof_state_tensor.view(self.num_envs, -1, 2)[:, :6, 0]
        diff = diff - self.dof_pos[:, :6]
        print(diff)
        #print(dof_state_tensor)
        #target_angle = angles + u.squeeze(-1) * 1  # 更新“比例”0.65，摩擦相关系数还有问题，摩擦正确的话1.0应该是可以的
        target_angle = self.dof_pos[:, :6] + u.squeeze(-1) * 1
        ...
        if self.ikcount == 20:
            diff = self.dof_pos[:, :6].clone()
            #print(diff)
            self.gym.refresh_dof_state_tensor(self.sim)  # 这行代码的问题
            #dof_state_tensor = self.gym.acquire_dof_state_tensor(self.sim)
            #dof_state_tensor = gymtorch.wrap_tensor(dof_state_tensor)
            #angles = self.dof_state_tensor.view(self.num_envs, -1, 2)[:, :6, 0]
            angles = self.dof_state.view(self.num_envs, -1, 2)[:, :6, 0]
            # 这边应该是针对dofpos更新，或者说refresh能更新dofpos吗
            #print(diff)
            diff = diff - angles
            #print(diff)
            # print(dof_state_tensor)
            target_angle = angles + u.squeeze(-1) * 0
            self.ikcount = 0
            #print("11111111111")
        else:
            target_angle = self.dof_pos[:, :6] + u.squeeze(-1) * 0.05
        ...
        if self.ikcount == 20:
            ...
            #print(diff)
            # print(dof_state_tensor)
            #target_angle = self.dof_pos[:, :6] + u.squeeze(-1) * 0.6
            self.ikcount = 0
        target_angle = self.dof_pos[:, :6] + u.squeeze(-1) * 0.05
        self.dof_pos[:, :6] = target_angle
        self.ikcount += 1
    ...
    def orientation_error(self, target_quat, current_quat):
        # 误差 = conj(current) * target
        # 然后将误差四元数转换为旋转向量（近似）
        print(current_quat)
        quat_err = quat_mul(target_quat, quat_conjugate(current_quat))
        axis = quat_err[..., 1:]  # x, y, z 分量
        sign = torch.sign(quat_err[..., 0]).unsqueeze(-1)  # 保证朝向最短路径
        return 2.0 * sign * axis  # 旋转向量近似
    '''
    ...
    def pre_physics_step(self, actions):
        ...
        if self.block_gripper:
            ...
            fingers_ctrl = actions[:, -1] * 0.2  # limit maximum change in position
            fingers_width = self._get_fingers_width()  # 还没进行适配
            #print(fingers_ctrl)
            target_fingers_width = fingers_width + fingers_ctrl
            asin_val = torch.clamp((target_fingers_width - 0.018) / 0.11, -1.0, 1.0)
            target_finger_joint = 0.69 - torch.asin(asin_val)

            target_angles = torch.cat([
                target_angles,
                target_finger_joint.unsqueeze(-1).repeat(1, 6)
            ], dim=-1)

        '''
        # 处理夹爪控制，还没定义
        else:
            gripper_ctrl = actions[:, -1] * 0.2  # 限制夹爪变化幅度为二成
            current_width = self._get_fingers_width()  # 这个函数要看看

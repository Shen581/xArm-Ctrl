...
$$
\mathcal{B}
= \left\{(o_{b,t}, a_{b,t})\ \middle|\ b=1,\ldots,B,\ t=0,\ldots,T-1\right\}.
$$
...
\begin{aligned}
h_{b,t}
&= \bigl[\operatorname{Encoder}(o_{b,t})\bigr]_{\mathrm{CLS}}, \\
z_{b,t}
&= \operatorname{Projector}(h_{b,t}).
\end{aligned}
$$
...
$$
u_{b,t}=\operatorname{ActionEncoder}(a_{b,t}),
\qquad
u_{b,t}\in\mathbb{R}^{D}.
$$
...
$$
\hat{z}_{b,t+1}
= \Bigl[\operatorname{Predictor}\bigl(z_{b,0:t},\ u_{b,0:t}\bigr)\Bigr]_{t},
\qquad t=0,\ldots,H-1.
$$
...
$$
\begin{array}{c|ccc}
\mathrm{position} & 0 & 1 & 2 \\
\hline
\mathrm{latent} & z_{b,0} & z_{b,1} & z_{b,2} \\
\mathrm{action} & u_{b,0} & u_{b,1} & u_{b,2} \\
\mathrm{target} & z_{b,1} & z_{b,2} & z_{b,3} \\
\mathrm{prediction} & \hat{z}_{b,1} & \hat{z}_{b,2} & \hat{z}_{b,3}
\end{array}
$$
...
= \frac{1}{BHD}\sum_{b=1}^{B}\sum_{t=0}^{H-1}\sum_{d=1}^{D}
\left(\hat{z}_{b,t+1,d}-z_{b,t+1,d}\right)^2.
$$
...
$$
Z = \operatorname{transpose}_{0,1}(\mathrm{emb}) \in \mathbb{R}^{T\times B\times D}.
$$

代码每次 forward 采样 `M=num_proj` 个单位随机方向 $r_m \in \mathbb{R}^D$，把 latent 投影成一维：

$$
y_{t,b,m}=z_{t,b}^{\top}r_m,
\qquad m=1,\ldots,M.
$$
...
$$
\phi(\tau_k)=\exp\left(-\frac{\tau_k^2}{2}\right).
$$

代码中的误差项等价于：

$$
\operatorname{err}_{t,m,k}
= \left(\frac{1}{B}\sum_{b=1}^{B}\cos(\tau_k y_{t,b,m})-\phi(\tau_k)\right)^2
+ \left(\frac{1}{B}\sum_{b=1}^{B}\sin(\tau_k y_{t,b,m})\right)^2.
$$
...
= \frac{1}{TM}\sum_{t=0}^{T-1}\sum_{m=1}^{M}
B\sum_{k=1}^{K}w_k\operatorname{err}_{t,m,k}.
$$
...
$$
\mathcal{L}_{\mathrm{pred}}
= \frac{1}{B\cdot 3\cdot D}\sum_{b=1}^{B}\sum_{i=1}^{3}
\left\|p_{b,i}-z_{b,i}\right\|_2^2.
$$
...
= \operatorname{SIGReg}\!\left([z_0,z_1,z_2,z_3]_{\mathrm{batch}}\right),
\qquad
\mathcal{L}
= \mathcal{L}_{\mathrm{pred}}+\lambda\mathcal{L}_{\mathrm{sigreg}}.
$$
...
$$
J(a_{0:T-1})
= \left\|z_{\mathrm{goal}}-\hat{z}_{\mathrm{final}}(a_{0:T-1})\right\|_2^2.
$$
...
$$
\mathcal{E}
= \operatorname{TopK}_{\mathrm{low}}
\left(\left\{J\!\left(a^{(s)}_{0:T-1}\right)\right\}_{s=1}^{S};\ K=30\right).
$$

然后更新采样分布：

$$
\mu \leftarrow \operatorname{mean}(\mathcal{E}),
\qquad
\sigma \leftarrow \operatorname{std}(\mathcal{E}).
$$

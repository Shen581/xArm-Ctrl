...
把代码里的训练过程写成公式，可以分成五步。设一个 minibatch 为：

$$ \mathcal{B} = \{(o_{b,t}, a_{b,t}) \mid b=1,\ldots,B,\ t=0,\ldots,T-1\}. $$

这里的 $a_{b,t}$ 不是单个原始环境动作，而是 `frameskip` 个原始动作拼成的 action block。

第一，图像 encoder 和 projector 得到 latent：

$$ h_{b,t} = [\mathrm{Encoder}(o_{b,t})]_{\mathrm{CLS}}. $$

$$ z_{b,t} = \mathrm{Projector}(h_{b,t}). $$

第二，action encoder 把 action block 变成和 latent 同维度的条件向量：

$$ u_{b,t}=\mathrm{ActionEncoder}(a_{b,t}), \qquad u_{b,t}\in\mathbb{R}^{D}. $$

第三，causal predictor 根据历史 latent 和 action embedding 做 next-embedding prediction：

$$ \hat{z}_{b,t+1} = [\mathrm{Predictor}(z_{b,0:t},\ u_{b,0:t})]_{t}, \qquad t=0,\ldots,H-1. $$

在默认 `history_size=3`、`num_preds=1` 时，代码实际对齐为：

| 项 | 位置 0 | 位置 1 | 位置 2 |
| --- | --- | --- | --- |
| latent context | $z_{b,0}$ | $z_{b,1}$ | $z_{b,2}$ |
| action context | $u_{b,0}$ | $u_{b,1}$ | $u_{b,2}$ |
| target | $z_{b,1}$ | $z_{b,2}$ | $z_{b,3}$ |
| prediction | $\hat{z}_{b,1}$ | $\hat{z}_{b,2}$ | $\hat{z}_{b,3}$ |

第四，预测损失是所有 batch、时间和维度上的 MSE：

$$ \mathcal{L}_{\mathrm{pred}} = \frac{1}{BHD}\sum_{b=1}^{B}\sum_{t=0}^{H-1}\sum_{d=1}^{D}(\hat{z}_{b,t+1,d}-z_{b,t+1,d})^2. $$
...
$$ Z = \mathrm{transpose}_{0,1}(\mathrm{emb}) \in \mathbb{R}^{T\times B\times D}. $$

代码每次 forward 采样 `M=num_proj` 个单位随机方向 $r_m \in \mathbb{R}^D$，把 latent 投影成一维：

$$ y_{t,b,m}=z_{t,b}^{\top}r_m, \qquad m=1,\ldots,M. $$
...
$$ \mathrm{err}_{t,m,k} = (\frac{1}{B}\sum_{b=1}^{B}\cos(\tau_k y_{t,b,m})-\phi(\tau_k))^2 + (\frac{1}{B}\sum_{b=1}^{B}\sin(\tau_k y_{t,b,m}))^2. $$

再用梯形积分权重加权，并乘以 batch size：

$$ \mathrm{SIGReg}(Z) = \frac{1}{TM}\sum_{t=0}^{T-1}\sum_{m=1}^{M} B\sum_{k=1}^{K}w_k\mathrm{err}_{t,m,k}. $$

所以总训练目标是：

$$ \mathcal{L} = \mathcal{L}_{\mathrm{pred}} + \lambda\mathrm{SIGReg}(Z). $$
...
$$ \mathcal{L}_{\mathrm{pred}} = \frac{1}{B\cdot 3\cdot D}\sum_{b=1}^{B}\sum_{i=1}^{3}\|p_{b,i}-z_{b,i}\|_2^2. $$

如果加上 SIGReg：

$$ \mathcal{L}_{\mathrm{sigreg}} = \mathrm{SIGReg}([z_0,z_1,z_2,z_3]_{\mathrm{batch}}), \qquad \mathcal{L} = \mathcal{L}_{\mathrm{pred}}+\lambda\mathcal{L}_{\mathrm{sigreg}}. $$
...
$$ a^{(s)}_{0:T-1} \sim \mathcal{N}(\mu,\sigma^2), \qquad s=1,\ldots,S. $$
...
$$ J(a_{0:T-1}) = \|z_{\mathrm{goal}}-\hat{z}_{\mathrm{final}}(a_{0:T-1})\|_2^2. $$

其中 $\hat{z}_{\mathrm{final}}$ 来自 `JEPA.rollout` 的最后一个 predicted embedding。CEM 选择 cost 最低的 `topk=30` 条 elite：

$$ \mathcal{E} = \mathrm{TopK}_{\mathrm{low}}(\{J(a^{(s)}_{0:T-1})\}_{s=1}^{S};\ K=30). $$

然后更新采样分布：

$$ \mu \leftarrow \mathrm{mean}(\mathcal{E}), \qquad \sigma \leftarrow \mathrm{std}(\mathcal{E}). $$
